...
public class SalaService {
    ...
    public SalaInfoDTO crearSala(Integer quizId, String hostEmail) {
    ...
    public UnirseResponseDTO unirse(String codigo, String nickname, String email) {
        ...
        return new UnirseResponseDTO(p.getId(), info);
    }
    ...
    public void responder(String codigo, Long participanteId, String respuesta) {
        ...
        if (p.isRespondioActual()) return; // ya respondió
        ...
        boolean correcta = pregunta.getRespuestaCorrecta().equalsIgnoreCase(respuesta);
        if (correcta) {
        ...
        // Notificar progreso de respuestas
        ...
        // Si todos han respondido → enviar resultado automáticamente
        if (respondidos == sala.getParticipantes().size()) {
            enviarResultado(sala, pregunta);
        }
    }
    ...
    public void siguiente(String codigo, String hostEmail) {
        ...
        List<Pregunta> preguntas = preguntaRepository.findByQuizIdOrderByOrden(sala.getQuiz().getId());
        Pregunta preguntaActual = preguntas.get(sala.getPreguntaActualIdx());

        // Enviar resultado antes de avanzar (por si el host forzó sin que todos respondieran)
        enviarResultado(sala, preguntaActual);
    ...
    private void enviarResultado(Sala sala, Pregunta pregunta) {
    ...
    private void verificarHost(Sala sala, String email) {
        if (!sala.getHost().getEmail().equals(email))
            throw new RuntimeException("Solo el host puede realizar esta acción");
    }

    private Sala getSala(String codigo) {
    ...
    private SalaInfoDTO toSalaInfo(Sala sala) {
        List<Pregunta> preguntas = preguntaRepository.findByQuizIdOrderByOrden(sala.getQuiz().getId());
